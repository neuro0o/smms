...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($type == 'activity') {
        ...
        VALUES ('$reviewText', '$reviewRating', '$reviewDate', '$userID', '$itemID', (SELECT apID FROM activity_purchase WHERE activityID = '$itemID' AND userID = '$userID' LIMIT 1))
        ";
    } elseif ($type == 'food') {
        $insertQuery = "
        INSERT INTO review_food (reviewText, reviewRating, reviewDate, reviewedBy, foodID, fpID)
        VALUES ('$reviewText', '$reviewRating', '$reviewDate', '$userID', '$itemID', (SELECT fpID FROM food_purchase WHERE foodID = '$itemID' AND userID = '$userID' LIMIT 1))
        ";
