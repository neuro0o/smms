        ...
        if ($result_accommodation) {
            if (mysqli_num_rows($result_accommodation) > 0) {
                while ($row = mysqli_fetch_assoc($result_accommodation)) {
                    echo "&nbsp;&nbsp;&nbsp;&nbsp;" . "<a class='searchResult' href='" . BASE_URL . "/MODULES/ACCOMMODATION_MODULE/accommodationList.php?id=" . htmlspecialchars($row['accommodationID']) . "'>" . htmlspecialchars($row['accommodationName']) . "</a><br>";
                }
            }
        }
        ...
        if ($result_activity) {
            if (mysqli_num_rows($result_activity) > 0) {
                while ($row = mysqli_fetch_assoc($result_activity)) {
                    echo "&nbsp;&nbsp;&nbsp;&nbsp;" . "<a class='searchResult' href='" . BASE_URL . "/MODULES/ACTIVITY_MODULE/activityList.php?id=" . htmlspecialchars($row['activityID']) . "'>" . htmlspecialchars($row['activityName']) . "</a><br>";
                }
            }
        }
