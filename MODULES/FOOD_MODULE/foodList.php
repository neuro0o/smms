<?php
  ...
  $categoryID = isset($_GET['categoryID']) ? $_GET['categoryID'] : 1; // Default to category 1
      ...
      <div class="foodCategorySection">
        <?php while ($categoryRow = mysqli_fetch_assoc($categoryResult)): ?>
          <div class="foodCategoryItem">
            <a href="foodList.php?categoryID=<?= $categoryRow['categoryID']; ?>" class="categoryLink"><?= $categoryRow['categoryName']; ?></a>
          </div>
        <?php endwhile; ?>
      </div>
      ...
      <div class="foodList">
        <?php while($row = mysqli_fetch_assoc($result)): ?>
          <div class="foodItem">
            ...
            <h3>
              <a href="javascript:void(0);" onclick="openFoodPopup(<?= htmlspecialchars(json_encode($row)); ?>)" class="food-name"><?= $row['foodName']; ?></a>
            </h3>
