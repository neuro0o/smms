...
$itemID = isset($_GET['id']) ? $itemID = $_GET['id'] : '';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($type == 'activity') {
        // Get the activityID using lineID
        $insertQuery = "
        ...
        VALUES ('$reviewText', '$reviewRating', '$reviewDate', '$userID', 
                (SELECT activityID FROM activity_purchase_detail WHERE lineID = '$itemID' LIMIT 1), 
                (SELECT apID FROM activity_purchase_detail WHERE lineID = '$itemID' LIMIT 1)
        )";
    } elseif ($type == 'food') {
        // Get the foodID using lineID
        $insertQuery = "
        INSERT INTO review_food (reviewText, reviewRating, reviewDate, reviewedBy, foodID, fpID)
        VALUES ('$reviewText', '$reviewRating', '$reviewDate', '$userID', 
                (SELECT foodID FROM food_purchase_detail WHERE lineID = '$itemID' LIMIT 1), 
                (SELECT fpID FROM food_purchase_detail WHERE lineID = '$itemID' LIMIT 1)
        )";
        ...
        VALUES ('$reviewText', '$reviewRating', '$reviewDate', '$userID', 
                (SELECT accommodationID FROM reservation WHERE reservationID = '$itemID' LIMIT 1), 
                '$itemID'
        )";
    } else {
    ...
    if (mysqli_query($conn, $insertQuery)) {
        ...
        header("Location: ../RESERVATION_MODULE/purchase_history.php");
        exit();
    } else {
...
$itemDetailsQuery = '';
if ($type == 'activity') {
    $itemDetailsQuery = "SELECT activityName FROM activity WHERE activityID = (SELECT activityID FROM activity_purchase_detail WHERE lineID = '$itemID' LIMIT 1)";
} elseif ($type == 'food') {
    $itemDetailsQuery = "SELECT foodName FROM food WHERE foodID = (SELECT foodID FROM food_purchase_detail WHERE lineID = '$itemID' LIMIT 1)";
} elseif ($type == 'accommodation') {
    $itemDetailsQuery = "SELECT accommodationName FROM accommodation WHERE accommodationID = (SELECT accommodationID FROM reservation WHERE reservationID = '$itemID' LIMIT 1)";
}
...
if ($itemDetails === null) {
    $itemName = "Unknown Item"; // Default value if no item found
} else {
    // Access the item name based on the type
    $itemName = $itemDetails['activityName'] ?? $itemDetails['foodName'] ?? $itemDetails['accommodationName'];
}
?>
...
<body>
    ...
    <div class="review-container">
        ...
        <p><strong>Reviewing: </strong><?php echo $itemName; ?></p>
    </div>
