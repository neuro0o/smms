...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Review submitted successfully!'); window.location.href='../RESERVATION_MODULE/reservation.php';</script>";
    } else {
...
<head>
    ...
    <style>
        ...
        textarea {
            ...
            padding: 10px;
            font-size: 14px;
            border: 1px solid #008080; /* Teal border for the textarea */
            border-radius: 5px;
        }
        select, textarea, input {
            width: 100%;
            padding: 10px;
            border: 1px solid #008080; /* Teal border for inputs */
            border-radius: 5px;
            font-size: 14px;
        }
        button {
            padding: 10px 20px;
            background-color: #008080; /* Teal background for the button */
            color: #fff;
            border: none;
            border-radius: 5px;
            font-size: 16px;
...
<body>
    <div class="container">
        ...
        <form action="review.php?type=<?php echo $type; ?>&id=<?php echo $itemID; ?>" method="POST">
            ...
            <textarea name="reviewText" id="reviewText" required placeholder="Share your experience..."></textarea>
